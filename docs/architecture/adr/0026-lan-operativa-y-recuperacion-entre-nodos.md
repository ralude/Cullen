...
D3 está implementada salvo la compensación explícita: compra transporta
`PurchaseReceiptCompleted.v1`, conteo `StockCountApproved.v1` y devolución
`SaleReturned.v2`; los tres aplican su movimiento
...
restituye stock. La conciliación distingue los tres resultados remotos y los cortes entre cada
frontera están probados. La **compensación explícita** de un rechazo definitivo **no** está
...
y los cortes entre cada frontera con reinicio de ambos nodos y reentrega del mismo `eventId`.
FS-011 conserva la brecha explícita de la compensación.
